...
"warning",function(err){feedback().warning(err,feedbackOptions)}).on("error",function(err){feedback().error(err,feedbackOptions)}).on("render",function(){var self=this;this.$component.addClass("modal").on("close.modal",function(){self.$component.modal("close")}).modal({width:600})}).render("body")}}),define("taoEventLog/controller/TaoEventLog/show",["jquery","lodash","i18n","util/url","ui/datatable","tpl!taoEventLog/controller/TaoEventLog/show/layout","taoEventLog/components/export/modalExporter","ui/dateRange/dateRange"],function($,_,__,url,datatable,layoutTpl,exporter,dateRangeFactory){"use strict";const listUrl=url.route("search","TaoEventLog",
"taoEventLog");return{start:function(){var isFromPortal="1"===$(".content").attr("data-is-from-portal"),dataTypes=[{key:"event_name",title:__("Event Name")},{key:"action",title:__("Action")},{key:"user_id",title:__("User ID")},{key:"user_login",title:__("User Login")}],datatableModel=[{id:"identifier",label:__("ID"),transform:function(id,row){return row.raw.id}},{id:"event_name",label:__("Event Name"),sortable:!0,filterable:!0},{id:"action",label:__("Action"),sortable:!0,filterable:!0},{id:"user_id",label:__("User ID"),sortable:!0,filterable:!0},{id:"user_login",label:__("User Login"),sortable:!0,filterable:!0}],isFromPortal=(isFromPortal||(dataTypes.push({
key:"user_roles",title:__("User Roles")}),datatableModel.push({id:"user_roles",label:__("User Roles"),sortable:!0,filterable:!0,transform:function(roles){var rolesArray=roles.split(", "),rolesCount=rolesArray.length;let result;return 1<rolesCount?result=currentFilter.filtercolumns.user_roles&&(rolesArray=_.find(rolesArray,function(item){return-1<item.toLowerCase().indexOf(currentFilter.filtercolumns.user_roles.toLowerCase())}))?__("%s and %s roles",rolesArray,rolesCount-1):__("%s roles",rolesCount):1===rolesCount&&(result=roles),result}})),dataTypes.push({key:"occurred",title:__("Occurred")},{key:"properties",title:__("Properties")}),datatableModel.push({
id:"occurred",label:__("Occurred"),sortable:!0}),{dataTypes:dataTypes});const $layout=$(layoutTpl(isFromPortal));dataTypes=$(".log-browser .log-table-filters",$layout);const $eventList=$(".log-browser .log-table",$layout),$eventViewer=$(".event-viewer",$layout);isFromPortal=$(".js-export",$layout);const filterRange=dateRangeFactory(dataTypes);let currentFilter={filtercolumns:{}};filterRange.on("submit reset",function(){$eventList.datatable("options",{params:{periodStart:this.getStart(),periodEnd:this.getEnd()}}),$eventList.datatable("refresh")}),isFromPortal.on("click",function(){currentFilter.filtercolumns=_.merge(currentFilter.filtercolumns,{
from:filterRange.getStart(),to:filterRange.getEnd()}),exporter({title:__("Export Log Entries"),exportUrl:url.route("export","TaoEventLog","taoEventLog",currentFilter)})}),$(".content").append($layout),$eventList.datatable({url:listUrl,sortby:"occurred",sortorder:"desc",filter:!0,rowSelection:!0,filterStrategy:"multiple",model:datatableModel,listeners:{selected:function(e,event){!function(event){let key,json;var str;for(key in event)event.hasOwnProperty(key)&&("properties"===key?(null!==(json=JSON.parse(event[key]))&&"object"!=typeof json&&(json=JSON.parse(json)),str=JSON.stringify(json,null,2),$("."+key,$eventViewer).html("<pre>"+str+"</pre>")
):"user_roles"===key?$("."+key,$eventViewer).html(event.user_roles.split(",").join("<br>")):$("."+key,$eventViewer).html(event[key]))}(event.raw),$eventViewer.is(".hidden")&&($layout.find(".col-12").removeClass("col-12").addClass("col-7"),$eventViewer.removeClass("hidden"))},filter:function(e,options){options.page=1},query:function(e,ajaxConfig){currentFilter=_.pick(ajaxConfig&&ajaxConfig.data,["filterquery","filtercolumns","sortby","sortorder","sorttype"])}}})}}}),define("taoEventLog/loader/taoEventLog.bundle",function(){});
